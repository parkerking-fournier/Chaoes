...
math::Vec3d
AttractorFunctions::king_fournier(const math::Vec3d& v, double dt) {
  // do nothing
  double temp = dt;
  temp = temp/1;
  return v;
...
AttractorFunctions::for_string(const std::string& name) {
  if (name == "king_fournier") return king_fournier;
  if (name == "aizawa") return aizawa;
  if (name == "bouali") return bouali;
  if (name == "burke_shaw") return burke_shaw;
  if (name == "chen_celikovsky") return chen_celikovsky;
  if (name == "chen_lee") return chen_lee;
  if (name == "dequan_li") return dequan_li;
