...
void
CurveGenerator::generate_king_fournier(double starting, double ending, float step) {
  //std::cout << "Generating King-Fournier curve with " << options_.iterations
      //<< " samples" << std::endl;

  float num_points = (starting-ending)/step;
    if(num_points < 0){num_points = -1*num_points;}
  float starting_position;
  float threshold = 222000;

  //Resize curve to the (length of iteration) * (number of points)
  curve_.resize(options_.iterations*num_points);

  //Iterate through each starting point
  for (std::size_t n = 0; n < num_points; ++n) {  
    starting_position = starting + step*n;
    std::vector<double> fiveD_point = {starting_position, starting_position, starting_position, starting_position, starting_position};

    //Disregard the first values
    for (std::size_t i = 0; i < threshold; ++i) {
      fiveD_point = KingFournierAttractor::king_fournier(fiveD_point, options_.delta);
    }
  
    // After threshold record the values into the curve
    Point point = Point(0.0);
    for (std::size_t i = 0; i < options_.iterations; ++i) {
      fiveD_point = KingFournierAttractor::king_fournier(fiveD_point, options_.delta);
      point[0] = fiveD_point[0];
      point[1] = fiveD_point[1];
      point[2] = fiveD_point[2];
      curve_[n*options_.iterations + i] = point;
    }
  }  
}
