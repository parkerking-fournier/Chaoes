...
main (int argc, char** argv)
{
  ...
  util::Arguments args;
  args.set_usage(argv[0], "[ OPTIONS ] OUTPUT_MESH.ply");
  ...
  args.add_option('o', "starting", true, "The x,y and z starting position of the attractor [0.95]");
  args.add_option('e', "ending", true, "The x,y and z ending position of the attractor [0.99]");
  args.add_option('w', "step", true, "The step size of steps between starting points. [0.0001]");
  ...
  while (util::ArgResult const* arg = args.next_option()) {
    ...
    } else if (arg->opt->lopt == "starting") {
      settings.starting = arg->get_arg<double>();
    } else if (arg->opt->lopt == "ending") {
      settings.ending = arg->get_arg<double>();
    } else if (arg->opt->lopt == "step") {
      settings.step = arg->get_arg<float>();
    }else {
  ...
  if(settings.function == "king_fournier"){
    generator.clear();
    generator.generate_king_fournier(settings.starting, settings.ending, settings.step);
  }
  else{
    generator.generate();
  }
